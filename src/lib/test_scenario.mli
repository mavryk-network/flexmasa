...
module Network : sig
  ...
  (** Start the nodes, bootstrap the client, and (potentially) activate the
      ...
      - [?do_activation]: if [true] the start-up will try to activate the
        protocol, if [false] it will do it only if the current level of the
        chain is [0]. *)
end
...
module Queries : sig
  val all_levels :
    ...
    ( (string * [> `Failed | `Level of int | `Null | `Unknown of string ]) list,
      [> System_error.t ] )
    Asynchronous_result.t
  (** Get the current chain level for all the nodes, returns {i node-ID × level}
      values. *)
  ...
  val run_wait_level :
    ...
    ( unit,
      ...
      | `Waiting_for of string * [ `Time_out ] ] )
    Asynchronous_result.t
  (** Runs wait_for_all_levels_to_be and provides [~seconds] by querying the
      node for time_between_blocks. *)
