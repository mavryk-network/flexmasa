...
let wait_for ?(attempts_factor = 0.) ?(silent = false) state ~attempts ~seconds
  ...
  let rec attempt nth =
    ...
    | `Not_done msg when nth < attempts ->
        ...
        >>= fun () ->
        System.sleep sleep_time >>= fun () -> again ()
...
module Netstat = struct
  let check_version state =
    Running_processes.run_cmdf state "netstat --version" >>= fun version_res ->
  ...
  let netstat_dash_nut state =
    check_version state >>= function
    | `Fine ->
        Running_processes.run_cmdf state "netstat -nut" >>= fun res ->
  ...
  let used_listening_ports state =
    netstat_dash_nut state >>= fun rows ->
...
module System_dependencies = struct
  ...
  let precheck ?(using_docker = false) ?(protocol_paths = [])
    ...
    (Netstat.check_version state >>= function
    ...
    | more, `Or_fail -> (
        let is_not_just_a_warning = function
          ...
          | `Missing_exec _ | `Not_a_protocol_path _ -> true
        in
        Console.sayf state
          Fmt.(
            fun ppf () ->
             ...
             desc (shout "Warning:")
               (wf
                  "This does not look like a standard Linux-ish environment. \
                   ...
                   https://gitlab.com/mavryk-network/flexmasa/blob/master/README.md#macosx-users "))
        else return ())
        >>= fun () ->
        (if
         List.exists more ~f:(function
...
module Shell_environement = struct
  ...
  let write state { aliases } ~path =
    let content =
      String.concat ~sep:"\n"
        ([ "# Shell-environment generated by a flexmasa-sandbox" ]
