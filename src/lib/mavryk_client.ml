...
let wait_for_node_bootstrap state client =
  ...
  let rec loop nth =
    if nth >= attempts then failf "Bootstrapping failed %d times." nth
      ...
      | true -> return ()
      | false ->
          System.sleep Float.(0.3 + (of_int nth * 0.6)) >>= fun () ->
          loop (nth + 1)
  in
  loop 1
...
let rpc state ~client meth ~path =
  ...
  in
  successful_client_cmd state ~client args >>= fun res ->
  ...
  with e -> (
    try
      Ezjsonm.from_string (sprintf "[ %s ]" output) |> function
      | `A [ one ] -> return one
      | _ -> raise e
    with e ->
      say state
...
let parse_account ~name ~lines =
  Option.(
    prefix_from_list ~prefix:"Hash:" lines >>= fun pubkey_hash ->
...
let sign_multisig state client ~contract ~amt ~to_acct ~signer_name =
  ...
  in
  client_cmd state ~client params >>= fun (_, sign_res) ->
  return (String.concat ~sep:"" sign_res#out)
...
let hash_data state ?gas client ~data_to_hash ~data_type =
  ...
  let the_list' =
    match gas with
    | None -> the_list
    | Some g -> the_list @ [ "--gas"; Int.to_string g ]
  in
  successful_client_cmd state ~client the_list' >>= fun res ->
...
let multisig_storage_counter state client contract_id =
  ...
  in
  rpc state ~client `Get ~path >>= fun sto ->
...
module Ledger = struct
      ...
      return
        {
          ...
          chain =
            (let v = Re.Group.get matches 2 in
             ...
             else Some (Mavryk_base58_digest.Identifier.Chain_id.decode v));
  ...
  let show_ledger state ~client ~uri =
    ...
    (* TODO: Use --for-script when available *)
    >>= fun res ->
    list_known_addresses state ~client >>= fun known_addresses ->
  ...
  let get_authorized_key state ~client ~uri =
    ...
    let re_uri =
      Re.(
        compile (seq [ str "Authorized baking URI: "; group (rep1 any); eol ]))
    in
...
module Keyed = struct
  ...
  let forge_and_inject state keyed_client ~json =
    ...
    let decoded =
      Mavryk_base58_digest.Identifier.Ed25519.Signature.decode to_decode
    ...
    | `O _ -> (
        ...
        | `A trans_list ->
            ...
            let to_ints (strs : string list) : int list =
              List.map strs ~f:(fun s -> Int.of_string s)
            in
            let to_max_int (ints : int list) : int =
              List.fold ints ~init:0 ~f:(fun acc x -> Int.max acc x)
            in
            let counters = List.fold trans_list ~init:[] ~f:foldf in
            let counter_strs =
              List.map ~f:(fun v -> Jqo.get_string v) counters
            in
  ...
  let get_contract_id state client origination_hash =
    operations_from_chain state client >>= fun ops_json ->
