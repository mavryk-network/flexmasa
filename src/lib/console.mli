...
(** {!Cmdliner.Term.t} which configures the console interaction (e.g. the
    ["--color"] option). *)
...
module Prompt : sig
  ...
  val item :
    ...
    Asynchronous_result.t) ->
    item
  (** [item description command_aliases action] creates a command which performs
      [action]; action gets all the arguments parsed as S-Expressions. *)
