...
let start t process =
  ...
  let open_file f =
    System_error.catch
      ...
      ()
  in
  open_file (output_path t process `Stdout) >>= fun stdout ->
  open_file (output_path t process `Stderr) >>= fun stderr ->
  (match process.kind with
  | `Process_group | `Docker _ -> return process.command
  | `Process_group_script s ->
      let path = output_path t process `Script in
      System.write_file t path ~content:s >>= fun () ->
      return (process.command @ [ path ]))
  >>= fun actual_command ->
  System_error.catch
    (fun () ->
...
let start_full t process =
  ...
  State.add_process t process proc >>= fun () ->
  return { process; lwt = proc } >>= fun proc_state ->
  return (proc_state, proc_full)
...
let wait_all t =
  State.all_processes t >>= fun all ->
  List.fold all ~init:(return ()) ~f:(fun prevm one ->
      prevm >>= fun () ->
      wait t one >>= fun _ -> return ())
...
let run_cmdf ?(id_prefix = "cmd") state fmt =
  let get_file path =
    System_error.catch
      Lwt.(
        fun () ->
          Lwt_io.open_file ~mode:Lwt_io.input path >>= fun inchan ->
          let stream = Lwt_io.read_lines inchan in
          Lwt_stream.to_list stream >>= fun l ->
          Lwt_io.close inchan >>= fun () -> return l)
      ()
  in
  ksprintf
    (fun s ->
      ...
      start state proc >>= fun proc ->
      wait state proc >>= fun status ->
      get_file (output_path state proc.process `Stdout) >>= fun out_lines ->
      get_file (output_path state proc.process `Stderr) >>= fun err_lines ->
      return
        (object
  ...
  let run_cmdf ?(id_base = "async-cmd") state ~f fmt =
    ksprintf
      (fun s ->
        ...
        let proc =
          Process.make_in_session id `Process_group [ "sh"; "-c"; s ]
        in
        start_full state proc >>= fun (proc_state, proc) ->
        Dbg.e EF.(wf "Started async: %S" proc_state.process.id);
        f proc_state proc >>= fun res ->
  ...
  let fold_process (process : Lwt_process.process_full) ~init ~f =
    ...
    let rec go prev_m () =
      ...
      | [], [] -> (
          ...
          >>= function
          | `Left None -> (
              System_error.catch Lwt_stream.peek err >>= function
              | None -> return prev
              | Some _ -> go (return prev) ())
          | `Right None -> (
              System_error.catch Lwt_stream.peek out >>= function
              | None -> return prev
              | Some _ -> go (return prev) ())
          | _ -> go (return prev) ())
      | outl, errl -> (
