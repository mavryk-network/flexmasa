...
module Commands = struct
  ...
  let du_sh_root state =
    unit_loop_no_args
      ...
      (fun () ->
        ...
        >>= fun du ->
        Console.display_errors_of_command state du >>= function
        | true ->
            Console.say state
              EF.(
                desc (haf "Disk-Usage:")
  ...
  let curl_unit_display ?(jq = fun e -> e) ?pp_json state cmd ~default_port
      ...
      Option.value_map pp_json ~default:[] ~f:(fun _ ->
          [
            Sexp_options.make_option "raw"
              "Do not try to pretty print the output.";
          ])
    in
    ...
    Console.Prompt.unit_and_loop ~description:doc
      ...
      cmd
      (fun sexps ->
        Sexp_options.port_number state sexps ~default_port >>= fun port ->
        get_pp_json state sexps >>= fun pp_json ->
        Helpers.curl_rpc_cmd state ~port ~path >>= fun json_opt ->
        do_jq ~msg:doc state json_opt ~f:jq >>= fun processed_json ->
        Console.sayf state
          More_fmt.(
            fun ppf () ->
  ...
  let all_levels state ~nodes =
    unit_loop_no_args ~description:"Get all “head” levels of all the nodes."
      [ "al"; "all-levels" ] (fun () ->
        Test_scenario.Queries.all_levels state ~nodes >>= fun results ->
        Console.say state
          EF.(
            desc (af "Node-levels:")
              (list
    ...
    Console.Prompt.unit_and_loop
      ...
      | [ Atom name ] ->
          ...
          List.fold procs ~init:(return []) ~f:(fun prevm { process; lwt } ->
              ...
              Running_processes.run_cmdf state "tail %s" out >>= fun tailout ->
              Running_processes.run_cmdf state "tail %s" err >>= fun tailerr ->
              return
                EF.(
                  desc_list
        ...
        let balance block contract =
          let path =
            sprintf "/chains/main/blocks/%s/context/contracts/%s/balance" block
              contract
          in
          Helpers.curl_rpc_cmd state ~port ~path >>= fun jo ->
          do_jq state jo ~msg:"Getting balance" ~f:(fun j ->
              Jqo.get_string j |> Int.of_string)
        in
        List.fold contracts ~init:(return []) ~f:(fun prevm hsh ->
            prevm >>= fun prev ->
            balance (Int.to_string save_point) hsh >>= fun init ->
            balance "head" hsh >>= fun current ->
            return ((hsh, init, current) :: prev))
        >>= fun results ->
        Console.say state
          EF.(
            desc_list
    ...
    let details =
      ...
      let only_opt =
        make_option "only"
          ...
          "Restrict the clients by name."
      in
      let all =
        (match clients with [ _ ] -> [] | _ -> [ only_opt ])
        @ Option.value_map make_admin ~default:[] ~f:(fun _ ->
              [ make_option "admin" "Use the admin-client instead." ])
      in
      match all with [] -> None | _ -> Some (pp_options all)
    in
    Console.Prompt.unit_and_loop
      ~description:
  ...
  let arbitrary_commands_for_each_client ?make_admin
      ?(make_command_names =
        fun i -> [ sprintf "c%d" i; sprintf "client-%d" i ]) state ~clients =
      ...
      (fun sexps ->
        ...
        protect_with_keyed_client "manual-forge" ~client ~f:(fun () ->
            ...
            | Some acct -> (
                ...
                >>= fun counter_json ->
                let counter =
                  (Jqo.get_string counter_json |> Int.of_string) + 1
                in
                let json_template =
                ...
                | true ->
                    System.read_file state tmp >>= fun content ->
                    let cleaned_up =
    ...
    Console.Prompt.unit_and_loop
      ...
      (fun sexps ->
        let client =
          ...
          | Some c -> c
          | exception _ ->
              Fmt.kstr failwith "Wrong command line: %a" pp (List sexps)
        in
        match sexps with
        | Atom "help" :: __ ->
            Console.sayf state
              More_fmt.(
                let cmd ppf name desc options =
                  ...
                  vertical_box ~indent:2 ppf (fun ppf ->
                      ...
                      cut ppf ();
                      Sexp_options.pp_options options ppf ())
                in
                fun ppf () ->
        ...
        | Atom "batch" :: more_args ->
            get_batch_args state ~client all_opts more_args >>= fun ba ->
            run_actions state ~client ~nodes ~actions:[ ba ] ~rep_counter:1
        | Atom "multisig-batch" :: more_args ->
            get_multisig_args state ~client all_opts more_args >>= fun ma ->
...
module Pauser = struct
    ...
    Console.say state
      EF.(
        desc
          ...
          (list
             ~param:{ default_list with space_before_separator = false }
             msgs))
    >>= fun () ->
  ...
  let run_test state f ~pp_error () =
    let finish () =
      Console.say state EF.(af "Killing all processes.") >>= fun () ->
      Running_processes.kill_all state >>= fun () ->
    ...
    in
    let wait_on_signals () =
      System_error.catch Lwt_condition.wait cond >>= fun sig_name ->
      return (`Woken_up_by_signal sig_name)
    in
    Dbg.e
    ...
    let run_with_signal_catching ~name procedure =
      Lwt.(
        pick
          [
            ( wrap_result (fun () -> procedure ()) >>= fun res ->
    ...
    let rec protect ~name procedure =
              ...
              Console.say state
                EF.(
                  desc
                    ...
                    (custom (fun ppf -> Attached_result.pp ~pp_error ppf result)))
              >>= fun () -> return `Error_that_can_be_interactive)
      >>= fun todo_next ->
      ...
      | `Error_that_can_be_interactive | `Quit_with_error ->
          finish () >>= fun () -> Asynchronous_result.die 4
    in
    protect f ~name:"Main-function" >>= fun () ->
