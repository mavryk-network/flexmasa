...
module Key : sig
  (** Deterministically generate public/private key (hashes) from given strings. *)
  module Of_name : sig
...
(** Build a {!Genspio.EDSL.t} script which generates the bootstrap-parameters
    JSON file. *)
...
(** Run the script created by [ensure_script], i.e. create the JSON bootstrap
    parameters. *)

val cli_term : < manpager : Manpage_builder.State.t ; .. > -> t Cmdliner.Term.t
(** Create a [Cmdliner] term which configures protocol-parameters (e.g. options
    like ["--time-between-blocks"]). *)
