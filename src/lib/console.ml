...
let do_output t =
  Lwt.(
    fun () ->
      Lwt_io.write t.channel (Buffer.contents t.buffer) >>= fun () ->
...
module Prompt = struct
  ...
  let command ?(with_defaults = true) state ~commands =
    ...
    in
    let rec loop () =
      say state EF.(af "Please enter command:") >>= fun () ->
      System_error.catch Lwt_io.read_line Lwt_io.stdin >>= fun line ->
                  ...
                  say state
                    EF.(
                      desc (shout "Error in action:")
                        (custom (fun ppf ->
                             Attached_result.pp ppf
                               result (* Error.pp ppf err *)
...
let display_errors_of_command state ?(should_output = false) cmd =
  let outputs () =
    List.exists cmd#out ~f:Poly.(fun s -> String.strip s <> "")
  in
  let success =
