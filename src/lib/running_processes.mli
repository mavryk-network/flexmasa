...
(** The definition of a process, for now, a process within a process-group or a
    Docker container. *)
module Process : sig
  type kind =
...
val output_path :
  ...
  [ `Meta | `Stderr | `Stdout ] ->
  string
(** Return the path (within {!Paths}'s root-path) where the process writes its
    output or metadata. *)
...
module Async : sig
  val run_cmdf :
    ...
    format4 ->
    'c
  (** Run a shell command and run a function over the process data before
      waiting for its end. *)
  ...
  (** Fold over the output and error-output of a {!Lwt_process.process_full},
      and *)
end
