...
module Command = struct
  let run ?output_error_codes state ~input_file ~output_file () =
    ...
    let transformed, error_codes =
      Transform.strip_errors_and_annotations expr in
    ...
    | other ->
        System_error.fail "Don't know what to do with extension %S" other )
    >>= fun () ->
    Asynchronous_result.map_option output_error_codes ~f:(fun path ->
        List.fold error_codes ~init:(return [])
          ~f:(fun prevm (code, error_msg) ->
            ...
            >>= fun json ->
            return
              (Ezjsonm.(dict [("code", int code); ("value", json)]) :: prevl))
        >>= fun jsons ->
      ...
      IFmt.(
        fun ppf () ->
          vertical_box ppf (fun ppf ->
              wf ppf "Convertion+stripping: `%s` -> `%s`." input_file
                output_file ;
