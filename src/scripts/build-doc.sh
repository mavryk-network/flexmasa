...
if ! [ -f src/scripts/build-doc.sh ] ; then
    say "This script should run from the root of the mavbox tree."
    exit 1
fi
...
odoc html-frag src/doc/index.mld \
     -I _build/default/src/lib_base58_digest/.mavryk_base58_digest.objs/byte/ \
     -I _build/default/src/lib_mv1_crypto/.mavryk_mv1_crypto.objs/byte/ \
     -I _build/default/src/lib/.mavbox.objs/byte/ -o "$lib_index_fragment"
...
tail +$toc_spot  ./README.md \
    | sed 's@https://protocol.mavryk.org/mavbox/lib-index.html@./lib-index.html@' \
...
For convenience, here is the output of `mavbox mini-net --help`:

EOF
    echo '``````'
    ./mavbox mini-net --help=plain
    echo '``````'
...
For convenience, here is the output of `mavbox daemons-upgrade --help`:

EOF
    echo '``````'
    ./mavbox daemons-upgrade --help=plain
    echo '``````'
